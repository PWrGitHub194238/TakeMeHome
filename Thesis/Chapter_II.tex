...
\textbf{Sortowanie topologiczne} polega na takim posortowaniu wierzchołków, aby dla każdej pary $\left( v_{i}, v_{j} \right)$, w~przypadku istnienia krawędzi pomiędzy tymi wierzchołkami, prowadzącej z~$v_{i}$ do $v_{j}$, w~już posortowanym ciągu $v_{i}$ znajdował się przed wierzchołkiem, do którego dana krawędź prowadzi.
...
\begin{pseudokod}[!htbp]
	...
	\caption{
		KHAN-TOPOLOGICAL-SORT $\left( G \right)$
	}
	\label{alg:KhanTopologicalSort}
\end{pseudokod}

Algorytm działa niemal identycznie jak w~przeprowadzonym przez nas rozumowaniu.
...
W faktycznej implementacji, linijki $5$--$7$ zwykle zastępuje się operacją $\textrm{\textsc{EXTRACT-MIN}} \left( Q \right)$, gdzie $Q$ to nasza kolejka priorytetowa.
...
Analizując złożoność czasową algorytmu, widzimy że jego główna pętla $4$--$9$ wykonuje się dokładnie $\left| V \right|$ razy, za każdym razem usuwając z~kolejki priorytetowej dokładnie jeden węzeł, gdzie skanowane węzły nie powtarzają się\footnote{
...
Jedną ze struktur, przystosowanych do operacji charakterystycznych dla kolejki priorytetowej, jest kopiec (ang. \textit{heap}).
...
Strukturami bardzo podobnymi do kopców są drzewa $K$-arne~\cite[$3.1$]{TaDS}~--- należy wręcz powiedzieć, że kopce są \textbf{pełnymi drzewami} (ang. \textit{complete tree}), podczas gdy struktura zwykłego drzewa jest mniej rygorystyczna.
...
\begin{pseudokod}[!htbp]
	\DontPrintSemicolon
	\Begin{
		\ForEach{$ i \in \left\{ 0, \dots, a - 1 \right\} $}{
		...
		\While{Wszystkie kubełki nie są puste} {
			...
			\ForEach{$ i \in \left\{ 0, \dots, a \right\} $}{
...
Algorytm rozpoczyna pracę od skanowania wszystkich kubełków w kolejności od tego o~najmniejszym zakresie aż do kubełka $B \left[ a\right]$.
...
\begin{pseudokod}[!htbp]
	...
	\Begin{
		...
		\ForEach{$ i \in \left\{ 0, \dots, K \right\} $}{
...
Ostatecznie daje nam to złożoność algorytmu ograniczoną z~góry przez $O \left( m \cdot b + n \cdot \left( b + \frac{C}{b}\right)\right)$.
...
Kolejnym algorytmem, któremu poświęcimy naszą uwagę, będzie algorytm oparty na kubełkach wielopoziomowych (w naszym przypadku będą to dwa poziomy~\cite[$4.2$]{NetOpt}).
...
Analizując złożoność obliczeniową algorytmu Dijkstry opartego o~kubełki wielopoziomowe, podzielimy naszą analizę na dwie części: w~pierwszej z~nich pokażemy złożoność algorytmu, którego przykład przedstawiono na rysunku~\ref{fig:exampleDoubleLevelBucketsA}, zaś następnie uogólnimy algorytm na $k$ poziomów.
...
Podsumowując powyższe rozważania: algorytm Dijkstry, zbudowany w~oparciu o~strukturę kubełków dwupoziomowych, działa w~czasie nieprzekraczającym $O \left( \frac{n \cdot C}{d} + n \cdot d + m \right)$.
...
Algorytm wykonywania wszystkich operacji pozostaje ten sam. W~przypadku napotkania przez algorytm kubełka $B \left[ a  \right]$ ($a = \left \lceil \log_{2} \left( C \right) \right \rceil + 1$), traktuje go on jak zwykły kubełek o~niejednostkowym zakresie z (najprawdopodobniej) wieloma wierzchołkami w~środku.
...
Tak samo jak wykorzystaliśmy ideę algorytmu $DKM$ implementując kopce pozycyjne z~przepełnieniem, podobnie możemy skonstruować algorytm w~oparciu o~strukturę dwupoziomową zastosowaną w~algorytmie $DKD$ (ang. \textit{Dijkstra with Double-Level Buckets}).
...
\begin{pseudokod}[!htbp]
	...
	\Begin{
		$ \textrm{\textsc{INIT-GRAPH}} \left( G, s \right)$ \;
		$n \longleftarrow \left \lceil \log_{2} \left( C + 1 \right) \right \rceil$ \;
		$S \left[ 0 \right] \longleftarrow 1; \; S \left[ 1 \right] \longleftarrow 1 $\;
		\ForEach{$ i \in \left\{ 2, \dots, n+1 \right\} $}{
			$S \left[ i \right] \longleftarrow 2 \cdot S \left[ i-1 \right] $\;
		}
		$B_{\mathbb{L}} \left[ 0 \right] \longleftarrow \textrm{\textsc{SET-RANGE}} \left( 0, 0 \right); \; B_{\mathbb{U}} \left[ 0 \right] \longleftarrow \textrm{\textsc{SET-RANGE}} \left( C +1 , C + 1 \right)$\;
		\ForEach{$ i \in \left\{ 1, \dots, n \right\} $}{
			$B_{\mathbb{L}} \left[ i \right] \longleftarrow \textrm{\textsc{SET-RANGE}} \left( S \left[ i \right], S \left[ i + 1 \right] - 1 \right)$\tcc*[f]{Ustawia zakres kubełka o indeksie $i$ na}\;
			$B_{\mathbb{U}} \left[ i \right] \longleftarrow \textrm{\textsc{SET-RANGE}} \left( C + 1 + S \left[ i \right], C + 1 + S \left[ i + 1 \right] - 1 \right)$\tcc*[f]{$\left[ S \left[ i \right]; S \left[ i + 1 \right] - 1 \right]$}\;
		}
		$B \longleftarrow B_{\mathbb{L}}; \; B \longleftarrow \textrm{\textsc{INSERT}} \left( s \right)$\;
		\For{$i = 0$ \emph{\KwTo} $ n $}{
			\eIf{$\left| B \left[ i \right] \right| == 1$ lub zakres kubełka jest jednostkowy}{
				\ForEach{ $v_{j} \in B \left[ i \right] $}{
					Usuń $v_{j}$ z~$B \left[ i \right] $ \;
					\ForEach{$e_{jk} : v_{j} \overset{1} \leadsto v_{k}$}{
						$RELAX \left( v_{j}, v_{k} \right)$ \tcc*[f]{Jeśli $v_{k}.d$ się zmieniło, to przenieś $v_{k}$ do odpowiedniego kubełka.}\;
				...
				$ minDist \longleftarrow \min \left\{ v_{j}.d : v_{j} \in B \left[ i \right] \right\}$\;
				$B \left[ 0 \right] \longleftarrow \textrm{\textsc{SET-RANGE}} \left( minDist, minDist \right)$\;
				\ForEach{ $B \left[ j \right] : 1 \leqslant j < i $}{
					$B \left[ j \right] \longleftarrow \textrm{\textsc{SET-RANGE}} \left( minDist + S \left[ j \right], \min \left\{ minDist + S \left[ j + 1 \right] - 1, B \left[ i \right].rrange \right\} \right)$\;
				}
				$i \longleftarrow 0$\;
				Przenieś z $B \left[ i \right]$ wszystkie wierzchołki do odpowiednich kubełków i kontynuuj pętlę $12$--$24$\;
			}
		}
		Ustaw odpowiednie zakresy dla wszystkich kubełków $B \left[ 0, \dots, n \right]$\tcc*[f]{Patrz ,,Uwagi do algorytmu''.}\;
		$B \longleftarrow \left( B == B_{\mathbb{L}}\right) \; ? \; B_{\mathbb{U}} \; : \; B_{\mathbb{L}}$\;
		\eIf{$\exists i \in \left\{ 0, \dots, n \right\} B \left[ i \right] \neq \emptyset$}{
			Powróć do pętli w linii $12$--$24$\;
		}{
			Obie listy kubełków $B_{\mathbb{L}}$ oraz $B_{\mathbb{U}}$ są puste. Algorytm kończy pracę. \;
		}
	}
	\caption{
		DKX $\left( G, s \right)$
